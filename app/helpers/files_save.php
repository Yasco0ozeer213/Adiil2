...
function saveFile(): string | null
{
    ...
    if (move_uploaded_file($_FILES['file']['tmp_name'], ROOT . '/api/files/' . $name)) {
        return $name;
    }
...
function saveImage(): string | null
{
    ...
    // Retourne null si l'image n'en est pas une, ou si elle n'a pas pu être enregistrée.
...
{
    $filePath = ROOT . "/api/files/" . $fileName;
