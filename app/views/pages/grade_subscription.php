...
<head>
    ...
    <title><?= $title ?? 'Mon Adhésion - ADIIL' ?></title>
...
<div>
    <div>
        <table>
            ...
            <tbody>
                <tr>
                    <td>Grade <?= htmlspecialchars($grade['nom_grade']) ?></td>
                    <td>1</td>
                    <td><?= number_format($grade['prix_grade'], 2, ',', ' ') ?> €</td>
                    <td><?= number_format($grade['prix_grade'], 2, ',', ' ') ?> €</td>
                </tr>
            </tbody>
        </table>

        <h3>Total &nbsp : &nbsp<?= number_format($grade['prix_grade'], 2, ',', ' ') ?> €</h3>
    </div>
    ...
    <div>    
        ...
        <div id="carte_credit" class="mode_paiement_fields">
            <form method="POST" action="/grade/subscribe?id=<?= $id_grade ?>">
